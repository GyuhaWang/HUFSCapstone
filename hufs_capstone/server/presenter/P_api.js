...
const controller = {
  ...
  pet: async (req, expressRes) => {
    ...
    model.PetInfo.create({
      ...
      .then((dbRes) => {
        ...
        expressRes.status(201).json({ message: "반려동물 정보 저장 성공" });
      })
      .catch((error) => {
  ...
  upload: async (req, expressRes) => {
    try {
      // Check if a file is included in the request
      if (!req.file) {
        return expressRes.status(400).json({ error: "CSV 파일이 없습니다." });
      }
      ...
      csv
        ...
        .on("end", async () => {
          ...
          expressRes.status(201).json({ message: "CSV 업로드 & DB 저장 성공" });
        });
    } catch (error) {
      // Handle errors and send an error response
      console.error("Error uploading and processing CSV file:", error);
      expressRes.status(500).json({ error: "CSV 파일을 저장하지 못했습니다." });
    }
  },
